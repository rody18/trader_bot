...
brokerSrv = bs.Broker()
positions = brokerSrv.getPortfolio()
#pp.pprint(acc_data)
pp.pprint(positions)
# --------- STOPS ----------------------------------------------------------------
msgs={}
...
for instrument in positions:
    ticker=instrument['ticker']['name']
    longQty=instrument['quantity']
    ...
    if stop is not None:
        ...
        last_price = instrument['lastPrice'] #quotes_resp[ticker]['lastPrice']
        if last_price <= stop_limit:
            ...
            msg="OK - Stop: {0} Precio: {1} - Stop Qty: {2} Posición Qty: {3}".format(stop_limit, last_price, stop_qty, longQty)
    else:
        msg="Sin STOP!!!"
